...
def _get_filenames_and_classes(dataset_dir):
	...
	for directory in directories:
		...
		for filename in os.listdir(directory):
			...
			cnt += 1
		print('%s have %d images' % (directory, cnt))
			...
			for shard_id in range(_NUM_SHARDS):
				...
				with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
					...
					for i in range(start_ndx, end_ndx):
						...
						#print(filenames[i], class_name, class_id)
		sys.stdout.write('\n')
		sys.stdout.flush()
...
def _rename_class_image(dataset_dir):
	if not os.path.exists(dataset_dir):  #检测目录或文件是否存在
		return
	...
	for class_name in class_names:
		...
		for image in os.listdir(class_dir):
			old_name = os.path.join(class_dir, image)
			new_name = os.path.splitext(old_name)
			new_name = '%s%d%s' % (class_name, cnt, new_name[1])
			new_name = os.path.join(class_dir, new_name)
...
#print ('total image num: %d' % len(_get_filenames_and_classes('./flower_photos/')[0]))
#_rename_class_image('./flower_photos_rename/')